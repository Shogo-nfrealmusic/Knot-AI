...
export const SkeletonFour = () => {
  return (
    <div className="p-8 overflow-hidden h-full relative flex flex-col group">
      ...
      <Container className="mt-10 ml-4">New request</Container>
      <Container className="mt-4 ml-10 group-hover:border-warm transition duration-200 group-hover:scale-[1.02]">
        Assign owner
      </Container>
      <Container className="mt-4 ml-4">Send invoice</Container>
      <Cursor
        className="top-20 left-40 group-hover:left-32"
        textClassName="group-hover:text-warm"
        text="Ops"
      />
      <Cursor
        className="top-60 left-12 group-hover:left-32 group-hover:top-44"
        textClassName="group-hover:text-neutral-900"
        text="Automation"
      />
    </div>
  );
...
const Container = ({
  ...
  return (
    <div
      className={cn(
        "p-0.5 rounded-lg border border-neutral-200 bg-neutral-50 w-fit",
        className
      )}
    >
      <div
        className={cn(
          "h-10  text-xs px-2 text-neutral-700  rounded-[5px] flex items-center justify-center bg-white shadow-[0_1px_2px_rgba(0,0,0,0.06)]"
        )}
      >
        {children}
      </div>
...
const CircleWithLine = ({ className }: { className?: string }) => {
  ...
  return (
    <div className={cn("flex flex-col items-center justify-center", className)}>
      <div
        className={cn(`rounded-full h-3 w-3 border border-neutral-300 bg-white`)}
      />
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width="2"
      ...
      >
        ...
        <defs>
          ...
          >
            ...
            <stop offset="0.530519" stopColor="#d4d4d4" stopOpacity="0.5" />
            <stop offset="1" stopColor="#d4d4d4" stopOpacity="0.05" />
          </linearGradient>
        </defs>
      </svg>
    </div>
